...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const suggestionsEl = document.getElementById('plannerSuggestions');
  const clearBtn = document.getElementById('clearPlans');
  ...
  function renderPlans() {
    ...
    plans.forEach((p, idx) => {
      ...
      del.setAttribute('aria-label', `Slett plan ${idx + 1}`);
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (!destination) {
        if (errorEl) {
          errorEl.textContent = 'Vennligst fyll inn destinasjon.';
      ...
      if (suggestionsEl) {
        suggestionsEl.innerHTML = `<strong>Forslag:</strong> ${escapeHtml(suggestions.join(', '))}`;
      }
